...
function extendMaterial(BaseMaterial, cfg) {
  ...
  Object.entries(cfg.uniforms ?? {}).forEach(([key, value]) => {
    uniforms[key] = value !== null && typeof value === "object" && "value" in value ? value : { value };
  });

  let vert = `${cfg.header}\n${cfg.vertexHeader ?? ""}\n${baseVert}`;
  let frag = `${cfg.header}\n${cfg.fragmentHeader ?? ""}\n${baseFrag}`;

  for (const [includeLine, code] of Object.entries(cfg.vertex ?? {})) {
    vert = vert.replace(includeLine, `${includeLine}\n${code}`);
  }

  for (const [includeLine, code] of Object.entries(cfg.fragment ?? {})) {
    frag = frag.replace(includeLine, `${includeLine}\n${code}`);
  }
