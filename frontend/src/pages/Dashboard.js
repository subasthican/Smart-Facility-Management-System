...
const Dashboard = () => {
  ...
  return (
    <section className="sf-page relative overflow-hidden">
      ...
      <div className="relative mb-10 grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3">
        {cards.map((card, idx) => (
          <Link key={idx} to={card.link} className="no-underline">
            <article className="sf-card p-6 transition hover:-translate-y-1">
      ...
      {user?.role === "STUDENT" && (
        <div className="sf-card relative p-10 text-center">
          <h3 className="sf-title text-2xl">Ready to book a facility?</h3>
          <p className="mt-2 sf-subtitle">Create a new booking, then track it from your bookings page and notifications feed.</p>
          <div className="mt-5 flex flex-wrap justify-center gap-2">
            <Link to="/create-booking" className="sf-btn-primary no-underline">
              Create New Booking
            </Link>
            <Link to="/notifications" className="sf-btn-secondary no-underline">
              Check Notifications
            </Link>
          </div>
        </div>
